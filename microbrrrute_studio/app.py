...
class MbseqStudio(tk.Tk):
    ...
    def toggle_theme(self) -> None:
        ...
        if self.dark_mode.get():
            self.configure(bg='#2b2b2b'); style.theme_use('clam')
            ...
            self.configure(bg='#f0f0f0'); style.theme_use('default')
            for w in ['TFrame','TLabel','TCheckbutton','TRadiobutton','TLabelframe','TLabelframe.Label']:
        ...
        self._save_settings(); self.refresh_all()

    def refresh_all(self) -> None:
        self.refresh_grid(); self.refresh_keyboard(); self.refresh_raw(); self.refresh_status(); self.refresh_piano_roll()
    ...
    def _save_settings(self) -> None:
        try:
            d = {k: getattr(self, k).get() for k in ['volume','tempo','dark_mode','wave_shape','step_res']}
    ...
    def open_recent(self, path: str) -> None:
        if not Path(path).exists():
            messagebox.showerror('Error', 'File not found'); self.recent_files.remove(path); self._save_recent(); self._refresh_recent_menu(); return
        self._do_open(path)

    def refresh_status(self) -> None:
        p = str(self.file_path) if self.file_path else 'unsaved'; cur = self.cursor.get()+1; flag = ' *' if self.dirty else ''
        steps = self.steps(); out = any(s.note is not None and (s.note < MIN_PLAYABLE or s.note > MAX_PLAYABLE) for s in steps)
    ...
    def show_settings_dialog(self) -> None:
        win = tk.Toplevel(self); win.title('Settings'); win.geometry('300x250'); win.resizable(False, False); win.transient(self); win.grab_set()
        ...
        ttk.Label(f, text='Volume').pack(anchor='w'); ttk.Scale(f, from_=0.0, to=0.8, variable=self.volume, orient='horizontal').pack(fill='x', pady=5)
        ttk.Label(f, text='Tempo (BPM)').pack(anchor='w'); ttk.Spinbox(f, from_=30, to=300, textvariable=self.tempo).pack(anchor='w', pady=5)
        ttk.Button(f, text='Close', command=win.destroy).pack(side='bottom', pady=10)

    def show_randomizer_dialog(self) -> None:
        win = tk.Toplevel(self); win.title('Randomizer'); win.geometry('350x300'); win.resizable(False, False); win.transient(self); win.grab_set()
        f = ttk.Frame(win, padding=20); f.pack(fill='both', expand=True)
        root_var = tk.StringVar(value='C'); scale_var = tk.StringVar(value='Minor Pentatonic'); oct_var = tk.IntVar(value=2); fill_var = tk.IntVar(value=75)
        ttk.Label(f, text='Root').grid(row=0, column=0, sticky='w'); ttk.Combobox(f, textvariable=root_var, values=NOTE_NAMES, state='readonly').grid(row=0, column=1, pady=5)
        ttk.Label(f, text='Scale').grid(row=1, column=0, sticky='w'); ttk.Combobox(f, textvariable=scale_var, values=list(SCALES.keys()), state='readonly').grid(row=1, column=1, pady=5)
        ttk.Label(f, text='Octaves').grid(row=2, column=0, sticky='w'); ttk.Spinbox(f, from_=1, to=4, textvariable=oct_var).grid(row=2, column=1, pady=5)
        ttk.Label(f, text='Fill %').grid(row=3, column=0, sticky='w'); ttk.Spinbox(f, from_=10, to=100, textvariable=fill_var).grid(row=3, column=1, pady=5)
        def do_rand():
            rm = NOTE_NAMES.index(root_var.get()) + 36
            ivs = SCALES[scale_var.get()]; pool = [rm + o*12 + i for o in range(oct_var.get()) for i in ivs]
    ...
    def push_undo(self) -> None:
        ...
        self._redo.clear()
    def undo(self) -> None:
        if self._undo: self._redo.append(self._snapshot()); self.project.sequences = self._undo.pop(); self.mark_dirty(); self.refresh_all()
    def redo(self) -> None:
        if self._redo: self._undo.append(self._snapshot()); self.project.sequences = self._redo.pop(); self.mark_dirty(); self.refresh_all()
    ...
    def paste_selection(self) -> None:
        ...
        tokens = text.split(); if not tokens: return
        self.push_undo(); steps = self.steps(); start = self.cursor.get()
        for i, t in enumerate(tokens):
            idx = start + i; if idx >= MAX_STEPS: break
    ...
    def import_midi_file(self) -> None:
        ...
        if not p: return
        try:
            raw = import_midi(p); steps = [Step(note=n) for n in raw][:MAX_STEPS]
            self.push_undo(); self.project.sequences[int(self.slot.get())] = steps; self.mark_dirty(); self.cursor.set(0); self.refresh_all()
    ...
    def refresh_grid(self) -> None:
        steps = self.steps(); win_width = self.winfo_width(); btn_width = 65
        cols = max(1, (win_width - 40) // btn_width); children = self.grid_inner.winfo_children()
    ...
    def _on_piano_roll_scroll(self, event: tk.Event) -> None:
        if event.state & 0x0004:
            ...
            self._piano_roll_zoom = max(0.5, min(5.0, self._piano_roll_zoom)); self.refresh_piano_roll()
        else:
    ...
    def refresh_piano_roll(self) -> None:
        self.piano_roll.delete('all'); w, h = self.piano_roll.winfo_width(), self.piano_roll.winfo_height(); if w < 10: return
    ...
    def _on_piano_roll_click(self, event: tk.Event) -> None:
        ...
        step_idx = int(event.x / cell_w); note_val = int((h + off_y - event.y) / cell_h + MIN_PLAYABLE - 1)
        if 0 <= step_idx < len(steps) and MIN_PLAYABLE <= note_val <= MAX_PLAYABLE:
            self.select_step(step_idx); self.push_undo(); steps[step_idx].note = note_val; self.mark_dirty(); self.refresh_all()

    def _show_step_context_menu(self, event: tk.Event, idx: int):
        self.select_step(idx); m = tk.Menu(self, tearoff=0); s = self.steps()[idx]
        m.add_command(label='Set Rest', command=lambda: self.set_step_rest(idx)); m.add_separator()
        ...
        gm = tk.Menu(m, tearoff=0); [gm.add_radiobutton(label=f'{int(g*100)}%', command=lambda v=g: self.set_step_gate(idx, v)) for g in [0.25, 0.5, 0.82, 1.0]]
    ...
    def highlight_note(self, note: int) -> None:
        ...
        rect = self.key_rects[note]; rel = note - (self.root_note + self.octave_shift.get()*12); self.keyboard.itemconfig(rect, fill='#7CFC8A'); self.after(200, lambda: self.keyboard.itemconfig(rect, fill=('black' if rel in BLACK_OFFSETS else 'white')))
    def insert_note(self, note: int) -> None:
        self.preview_note(note); steps = self.steps(); idx = self.cursor.get(); if idx >= len(steps) and self._at_step_limit(): return
        self.push_undo()
    ...
    def change_slot(self) -> None: self.cursor.set(0); self.bank_name_var.set(self.project.bank_names.get(int(self.slot.get()), f'Bank {self.slot.get()}')); self.clear_selection(); self.refresh_all()
    ...
    def delete_step(self) -> None:
        ...
        self.push_undo()
        if self._selection: [steps.pop(idx) for idx in sorted(list(self._selection), reverse=True)]; self.clear_selection()
    ...
    def clear_slot(self) -> None: if messagebox.askyesno('Clear', 'Clear bank?'): self.push_undo(); self.project.sequences[int(self.slot.get())] = [Step() for _ in range(MAX_STEPS)]; self.mark_dirty(); self.cursor.set(0); self.refresh_all()
    def _do_open(self, p: str | Path) -> None:
        try: self.project = MbseqProject.load(p); self.file_path = Path(p); self.dirty = False; self._undo.clear(); self._redo.clear(); self.slot.set(1); self.bank_name_var.set(self.project.bank_names.get(1, 'Bank 1')); self.cursor.set(0); self.refresh_all(); self._add_recent(p)
        except Exception as e: messagebox.showerror('Error', str(e))
    def open_file(self) -> None: p = filedialog.askopenfilename(filetypes=[('MBSEQ','*.mbseq'),('All','*.*')]); if p: self._do_open(p)
    ...
    def save_as(self) -> None: p = filedialog.asksaveasfilename(defaultextension='.mbseq', filetypes=[('MBSEQ','*.mbseq')]); if p: self._do_save(p)
    def apply_raw(self) -> None:
        try: self.push_undo(); self.project = MbseqProject.parse(self.raw.get('1.0','end')); self.slot.set(1); self.bank_name_var.set(self.project.bank_names.get(1, 'Bank 1')); self.mark_dirty(); self.cursor.set(0); self.refresh_all()
        except Exception as e: messagebox.showerror('Error', str(e))
    ...
    def _start_playback(self) -> None:
        ...
        self.playing = True; self._play_idx = 0; if self.play_btn: self.play_btn.config(state='disabled')
        if self.stop_btn: self.stop_btn.config(state='normal')
        self.refresh_all(); data = render_steps_to_data(steps, bpm=self.tempo.get(), wave_shape=self.wave_shape.get(), volume=self.volume.get(), metronome=self.metronome.get())
        self._pre_render_file = Path(tempfile.gettempdir()) / f'mbseq_{uuid.uuid4().hex}.wav'; render_pre_rendered_wav(self._pre_render_file, data)
        if self.count_in.get(): self._play_count_in(8)
        else: self._start_audio_and_tick()
    def _start_audio_and_tick(self) -> None: if self.playing and self._pre_render_file: play_pre_rendered_wav(self._pre_render_file); self._play_tick()
    def _play_count_in(self, s: int) -> None:
        if not self.playing: return
        if s <= 0: return self._start_audio_and_tick()
        ms = int(30000 / max(1, self.tempo.get())); self.preview_note(84 if s % 2 == 0 else 72, 0.03); self._after_id = self.after(ms, lambda: self._play_count_in(s - 1))
    def _play_tick(self) -> None:
        ...
        self._playhead = self._play_idx; self.refresh_grid(); self.refresh_piano_roll(); n = steps[self._playhead].note
    ...
    def stop_sequence(self) -> None:
        self.playing = False; if self._after_id: self.after_cancel(self._after_id); self._after_id = None
        stop_all(); if self._pre_render_file: try: self._pre_render_file.unlink() except Exception: pass
        self._pre_render_file = None; self._playhead = -1; if self.play_btn: self.play_btn.config(state='normal')
    ...
    def _on_drag_motion(self, event: tk.Event) -> None:
        ...
        for i, btn in enumerate(self.step_buttons):
            if btn == target:
                # Highlight active drop target with a distinct color and border
                btn.config(highlightthickness=3, highlightbackground='#7CFC8A', bg='#e8ffea' if not self.dark_mode.get() else '#1a331c')
            else:
                # Restore normal/selected/cursor state colors
    ...
    def _on_drag_stop(self, event: tk.Event, src_idx: int) -> None:
        ...
        target = event.widget.winfo_containing(event.x_root, event.y_root); dst_idx = next((i for i, b in enumerate(self.step_buttons) if b == target), None)
        if dst_idx is not None and dst_idx != src_idx: self.push_undo(); steps = self.steps(); steps.insert(dst_idx, steps.pop(src_idx)); self.mark_dirty(); self.refresh_all()
        self._drag_idx = None
    ...
    def duplicate_bank_dialog(self) -> None:
        ...
        def do():
            ...
            self.push_undo(); self.project.sequences[target.get()] = [Step(s.note, s.gate, s.accent, s.slide) for s in self.steps()]; self.mark_dirty(); self.refresh_status(); win.destroy()
